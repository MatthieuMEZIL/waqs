...
if (-not (Test-Path $roslynAssemblies))
{
    ...
    if (Test-Path "HKCU:\Software\Microsoft\VisualStudio\14.0_Config")
    {
        ...
        [System.IO.File]::WriteAllText((Join-Path $ttIncludePath 'WAQS.Roslyn.Assemblies.ttinclude'), '<#@ assembly name="'+$assembliesPath+'\System.Reflection.Metadata.dll" #>'+[System.Environment]::NewLine + '<#@ assembly name="'+$assembliesPath+'\System.Collections.Immutable.dll" #>'+[System.Environment]::NewLine + '<#@ assembly name="'+$assembliesPath+'\Microsoft.CodeAnalysis.dll" #>'+[System.Environment]::NewLine + '<#@ assembly name="'+$assembliesPath+'\Microsoft.CodeAnalysis.CSharp.dll" #>'+[System.Environment]::NewLine + '<#@ assembly name="'+$assembliesPath+'\Microsoft.CodeAnalysis.Workspaces.dll" #>'+[System.Environment]::NewLine + '<#@ assembly name="'+$assembliesPath+'\Microsoft.CodeAnalysis.Workspaces.Desktop.dll" #>'+[System.Environment]::NewLine + '<#@ assembly name="'+$assembliesPath+'\Microsoft.CodeAnalysis.CSharp.Workspaces.dll" #>'+[System.Environment]::NewLine + '<#@ assembly name="'+$assembliesPath+'\System.Composition.TypedParts.dll" #>')
    }
    else
    {
